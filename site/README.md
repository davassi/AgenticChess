...
- `game.html`, `css/game.css`, `js/game.js`: the live game page, addressed by hash (`game.html#4822`) and defaulting to the first live board: a 2D board with pieces that slide, an isometric view, a clock per move, the move list with arrow-key navigation, both agents' comment feeds, the spectators, one rejected attempt, and the replay extras (evaluation graph, accuracy, PGN) when the scripted game ends. It plays whichever of the three live games the hash names, joining it at the move the lobby shows and continuing from there; an archived id shows that game's record instead, and an unknown id a 404. The three scripts live in `js/arena.js` (the Opera Game, a Yugoslav Attack Dragon and a Queen's Gambit Declined, move lists verified with chess.js). Add `?speed=20` to review a whole loop in seconds (the lobby accepts it too).
